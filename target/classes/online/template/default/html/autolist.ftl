...
function createDataGrid${config_id}(){
	...
	fit:true,
	<#--update-start--Author:Yandong  Date:20180111 for：TASK #2472 【online样式】Online 测试功能，字段太多，列表没有滚动条-->
	//fitColumns:true,
	<#--update-start--Author:Yandong  Date:20180111 for：TASK #2472 【online样式】Online 测试功能，字段太多，列表没有滚动条-->
	striped:true,
	autoRowHeight: true,
		...
		[	
			...
			{field:'opt',title:'操作',width:200,formatter:function(value,rec,index){
						...
						if(rec.bpm_status!=null&&rec.bpm_status.indexOf('提交')>0){
							href+="&nbsp;<a href='javascript:void(0)' class='ace_button' onclick=confirm('activitiController.do?startOnlineProcess&configId=${config_id}&id="+rec.id+"','确认提交流程','${config_id}List')>";
							href+="<i class='fa fa-download'></i>提交流程</a>";
						}
						
						//流程进度
						if(rec.bpm_status!=null &&(rec.bpm_status!='1' && rec.bpm_status!='待提交')&&(rec.bpm_status!='3' && rec.bpm_status!='已完成')){
						    href+="&nbsp;<a href='javascript:void(0)' class='ace_button' style='background-color: red' onclick=progress('"+rec.id+"')>";
							href+="<i class='fa fa-history'></i>流程进度</a>";
						}
						...
						<#list config_buttons as x>
							<#if x['buttonStyle'] == 'link' && x['buttonStatus']=='1' && config_noliststr?index_of("${x['buttonCode']}")==-1>
								...
								<#elseif brower_type?? && brower_type == 'Microsoft%20Internet%20Explorer'>
...
<#if  (config_queryList?size >0)>
	<#if config_querymode == "single">
		<span style="float:right">
			<#--update-begin--Author:dangzhenghui  Date:20170502 for：去掉查询默认条件-->
		<input id="${config_id}Listsearchbox" class="easyui-searchbox"  data-options="searcher:${config_id}Listsearchbox,prompt:'',menu:'#${config_id}Listmm'"></input>
			<#--update-end--Author:dangzhenghui  Date:20170502 for：去掉查询默认条件-->
