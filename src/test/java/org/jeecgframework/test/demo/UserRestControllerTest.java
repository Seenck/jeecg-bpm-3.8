...
public class UserRestControllerTest  extends AbstractUnitTest{
	...
	public void setup() throws Exception {
		...
		//update-start--Author:yugwu  Date:20170924 for：绕过后台拦截器校验及saveLoginSuccessInfo重置session
		Client c = new Client();
		TSUser u = new TSUser();
		u.setUserName("admin");
		c.setUser(u);
		session.setAttribute(session.getId(), c);
		//update-end--Author:yugwu  Date:20170924 for：绕过后台拦截器校验及saveLoginSuccessInfo重置session
