...
//	update-end--Author:zhoujf  Date:20150608 for：修改增加easyui css主题目录切换的属性
	...
	public void setFunUrl(String title, String exp, String funname,String operationCode, String urlStyle,String urlclass,String urlfont,boolean inGroup) {
		...
		dataGridUrl.setExp(exp);
		dataGridUrl.setFunname(funname);
		//update-begin-author:taoYan date:20180524 for:操作菜单扩展---
		dataGridUrl.setInGroup(inGroup);
		...
		if (!StringUtils.isBlank(dictionary)&&(!popup)) {
			if(dictionary.contains(",")){
				...
				String sql = "select " + dic[1] + " as field," + dic[2]
						+ " as text from " + dic[0];
				//-- update-begin--Author:zhoujf  Date:20180319 for：TASK #2560 【UI标签扩展】column列表字典属性扩展dictCondition加sql条件  --
				if(!StringUtil.isEmpty(dictCondition)){
					sql += " "+dictCondition;
				}
				//-- update-end--Author:zhoujf  Date:20180319 for：TASK #2560 【UI标签扩展】column列表字典属性扩展dictCondition加sql条件  --
				systemService = ApplicationContextUtil.getContext().getBean(
						SystemService.class);
...
//        log.info("================================ DataGridTag 开始时间:"+sdf.format(new Date())+"==============================");
		...
		try {
			...
			// update-begin-Author:zhoujf Date:20180504 for:TASK #2687 【datagrid标签】使用freemaker重写datagrid标签
			...
			// update-end-Author:zhoujf Date:20180504 for:TASK #2687 【datagrid标签】使用freemaker重写datagrid标签
			out.flush();
...
//         log.info("================================ DataGridTag 耗时:"+(end-start)+"ms==============================");
	...
	public StringBuffer jqGrid(){
		...
		//update--begin--author:zhoujf date:20170427 for:增加jqgrid列表展示形式
		...
		//update--end--author:zhoujf date:20170427 for:增加jqgrid列表展示形式
...
//                update-end-Author:zhangguoming  Date:20140921 for：TASK #458 列表hidden=false，才是隐藏好像有点问题
	...
	public StringBuffer end() {
		...
		if(!treegrid && isShowSubGrid){
			...
			//--update--begin--Author:Yandong  Date:20180410 for：TASK #2450 【改造】支持主子表效果
			...
			for (String ss : subtableids) {
				...
				for (Map<String, Object> map : subfieldlist) {
					...
					//--update--begin--Author:Yandong  Date:20180413 for：TASK #2635 【列表详情】主子表列表详情查询数据不对
//					if(map.get("main_field")!=null && map.get("main_field").toString().length()>0 && "".equals(linkid)){
//						linkid = (String)map.get("field_name");
//					}
					//--update--end--Author:Yandong  Date:20180413 for：TASK #2635 【列表详情】主子表列表详情查询数据不对
					//--update--begin--Author:Yandong  Date:20180410 for：TASK #2450 【改造】支持主子表效果
					columns += "{title:'"+map.get("content")+"',field:'"+map.get("field_name")+"',align:'left',width:50},";
					//--update--end--Author:Yandong  Date:20180410 for：TASK #2450 【改造】支持主子表效果
				}
				//--update--begin--Author:Yandong  Date:20180413 for：TASK #2635 【列表详情】主子表列表详情查询数据不对
...
//	    update-end--Author:chenjin  Date:20160715 for：扩展标签<t:dgCol 增加字段defaultVal=""		
		...
		//update-begin-author:taoyan date:20180522 for:TASK #2596 jeecg参考船舶一对多表单demo剩余任务----
		//高级查询避免方法名出现重复
		...
		//过滤操作
		getFilterFields(sb);
		//update-end-author:taoyan date:20180522 for:TASK #2596 jeecg参考船舶一对多表单demo剩余任务----
		sb.append("</script>");
		...
		//update-begin-author:taoYan date:20180327 for:高级查询需要提前，不一定要有查询 条件的时候才能用--
		...
		//update-end-author:taoYan date:20180327 for:高级查询需要提前，不一定要有查询 条件的时候才能用--
		...
		//update-end--Author:scott---date:20171109-----for:非组合查询情况，不生成隐藏查询区域------
			...
			//update--begin--author:zhangjiaqiang Date:20180223 for:TASK #2525 【浏览器】浏览器兼容问题
			sb.append("<span style=\"max-width: 79%;display: inline-block;\">");
			//update--end--author:zhangjiaqiang Date:20180223 for:TASK #2525 【浏览器】浏览器兼容问题
			
			//update--begin--author:scott----- Date:20180613------ for:TASK #2795 【改进】查询规则友好提醒
			...
			//update--end--scott----- Date:20180613------ for:TASK #2795 【改进】查询规则友好提醒
		...
		//update--begin--author:Yandong date:20180403 for:TASK #2605 【UI扩展】如何只要高级查询
		}else if(hasQueryColum(columnList) && "advanced".equals(getQueryMode())){
			blink = true;
			String searchColumStyle = toolBarList!=null&&toolBarList.size()!=0?"":"style='border-bottom: 0px'";
			//update--begin--author:zhoujf date:20180410 for:TASK #2605 【UI扩展】如何只要高级查询(多出一个线)
			sb.append("<div name=\"searchColums\" style=\"display:none;\" id=\"searchColums\" "+searchColumStyle+">");
			//update--end--author:zhoujf date:20180410 for:TASK #2605 【UI扩展】如何只要高级查询(多出一个线)
			sb.append("<input  id=\"isShowSearchId\" type=\"hidden\" value=\""+isShowSearch+"\"/>");
			//-----longjb1 增加用于高级查询的参数项 
			sb.append("<input  id=\"_sqlbuilder\" name=\"sqlbuilder\"   type=\"hidden\" />");
			sb.append("<form onkeydown='if(event.keyCode==13){" + name + "search();return false;}' id='"+name+"Form'>");
		...
		//update--end--author:Yandong date:20180403 for:TASK #2605 【UI扩展】如何只要高级查询
		//update-start--Author:scott---date:20171121-----for:没有按钮的时候，查询区域不生成底部线条----------------
		...
		//update--begin--author:zhangjiaqiang date:20171113 for:TASK #2416 【UI标签】查询按钮换下位置，放到查询条件后面，注意查询字段多的时候换行问题（查询按钮始终在最后面，靠右）
		 if("single".equals(getQueryMode())&& hasQueryColum(columnList)){//如果表单是单查询
			...
			for (DataGridColumn col : columnList) {
				if (col.isQuery()) {
					//update--begin--author:Yandong date:20180403 for:TASK #2608 【UI标签】datagrid 问题
					sb.append("<div data-options=\"name:\'"+col.getField().replaceAll("_","\\.")+"\',iconCls:\'icon-ok\' \">"+col.getTitle()+"</div>  ");
					//update--end--author:Yandong date:20180403 for:TASK #2608 【UI标签】datagrid 问题
				}
			}
			sb.append("</div>");
			sb.append("</span>");
	...
	//update-begin-author:taoYan date：20170717 for:TASK #2980 【BUG】分组按钮问题--加载单个toolbar按钮方法提取出来
	/**
	 * 仅用于 加载【更多操作】 按钮的方法
	 * @param sb 
	...
	//update-end-author:taoYan date：20170717 for:TASK #2980 【BUG】分组按钮问题---加载单个toolbar按钮方法提取出来
		...
		if("group".equals(getQueryMode()) && hasQueryColum(columnList)){//如果表单是组合查询
			...
			if (btnCls != null && !btnCls.equals("easyui")) {//自定以样式 bootstrap按钮样式
				if(btnCls.indexOf("bootstrap")==0){
					...
					// update-begin-Author:LiShaoQing Date:20171227 for:添加新的高级查询构造器功能-----
					if(oConvertUtils.isNotEmpty(complexSuperQuery)) {
						//update-begin-author:taoyan date:2180522 for:确保高级查询方法名称唯一----
						sb.append("<button class=\""+defalutCls+"\"  type=\"button\" onclick=\""+name+"SuperQuery('"+complexSuperQuery+"')\">");
						//update-end-author:taoyan date:2180522 for:确保高级查询方法名称唯一----
						sb.append("<i class=\"fa fa-search\"></i>");
		...
		if("group".equals(getQueryMode())){
			...
			for (DataGridColumn col : columnList) {
				if (col.isQuery()) {
					...
					//update--end--author:scott----- Date:20180613------ for:TASK #2795 【改进】查询规则友好提醒
...
//								    update-end--Author:chenjin  Date:20160718 for：扩展标签<t:dgCol 增加字段defaultVal=""		
							...
							//-- update-begin--Author:zhoujf  Date:20180330 for：datagrid，popup查询条件多字段回填问题--
							//-- update-begin--Author:zhoujf  Date:20180409 for：datagrid，popup查询条件多字段回填问题(机制改造)--
...
//								    update-end--Author:chenjin  Date:20160718 for：扩展标签<t:dgCol 增加字段defaultVal=""	
								//-- update-end--Author:zhoujf  Date:20180409 for：datagrid，popup查询条件多字段回填问题(机制改造)--
								//-- update-end--Author:zhoujf  Date:20180330 for：datagrid，popup查询条件多字段回填问题--
...
//							    update-end--Author:chenjin  Date:20160718 for：扩展标签<t:dgCol 增加字段defaultVal=""		
	...
	protected void getOptUrl(StringBuffer sb) {
		...
		for (DataGridUrl dataGridUrl : list) {
			if(!dataGridUrl.isInGroup()){
				...
				if (OptTypeDirection.OpenTab.equals(dataGridUrl.getType())) {
					//update-begin--Author: Yandong  Date:20180529 for：TASK #2744 【问题确认】论坛问题测试确认
					if(!StringUtil.isEmpty(dataGridUrl.getUrlclass())){//倘若urlclass不为空，则去掉链接前面的"[";
					...
					//update-end--Author: Yandong  Date:20180529 for：TASK #2744 【问题确认】论坛问题测试确认
				}
		...
		//update-begin-author：taoyan date:20180718 for:TASK #2980 【BUG】分组按钮问题--通用分组按钮的放在后面
		if(hasGroup()){
			getGroupUrl(sb);
		}
		//update-end-author：taoyan date:20180718 for:TASK #2980 【BUG】分组按钮问题--通用分组按钮的放在后面
	...
	/*	sb.append("href+=\"<div class='opts-menu-triangle icon-triangle' title='更多操作'></div>\";");*/
		...
		for (DataGridUrl dataGridUrl : list) {
			if(dataGridUrl.isInGroup()){
				...
				//update-start--Author: chenj  Date:20160815 for：TASK #1040 【UI按钮标签ace样式】列表后面的操作按钮支持按钮标签样式设置，
				StringBuffer urlclass = new StringBuffer();
				//update-begin-author:taoyan date:20180717 for:TASK #2980 【BUG】分组按钮问题
				if(StringUtil.isEmpty(dataGridUrl.getUrlclass())){
					dataGridUrl.setUrlclass("btn btn-default ops-more");
				}
				urlclass.append(" class=\'"+dataGridUrl.getUrlclass()+"\'");
				//update-end-author:taoyan date:20180717 for:TASK #2980 【BUG】分组按钮问题
				StringBuffer urlfont = new StringBuffer();
				if(!StringUtil.isEmpty(dataGridUrl.getUrlfont())){
					urlfont.append(" <i class=\' fa ");
					urlfont.append(dataGridUrl.getUrlfont());
...
//			      update-start--Author:chenjin  Date:20160811 for：TASK #1259 【论坛bug】论坛bug确认是否存在问题，存在进行处理,行编辑模式下，增加combobox
					...
					if(!StringUtil.isEmpty(column.getDictionary())){//根据数据字典生成editor:'combobox'
						if(column.getDictionary().contains(",")){
							...
							//update-end-Author:zhoujf  Date:20180724 for:TASK #2996 【友好提示】ajax模式的列表加载提示信息
									...
									//-- update-begin--Author:zhoujf  Date:20180409 for：数据字典没有转化值显示原值报错问题(补日志)
									for (Map<String, Object> map : list){
									...
									//-- update-end--Author:zhoujf  Date:20180409 for：数据字典没有转化值显示原值报错问题(补日志)
									comboboxStr.append("}");
								}
							}
							//update-end-Author:zhoujf  Date:20180712 for:TASK #2809 【bug】网友反馈问题，字典性能问题
				...
				//update--begin--author:zhoujf Date:20180319 for:行编辑扩展参数字典值为空的问题
				//update--begin--author:zhoujf date:20180330 for:TASK #2592 【代码生成器】单表档案行编辑风格表单，代码生成，点击页面编辑所有的数据不可操作
				String extendParm = "";
				...
				//update--end--author:zhoujf date:20180330 for:TASK #2592 【代码生成器】单表档案行编辑风格表单，代码生成，点击页面编辑所有的数据不可操作
				//update--end--author:zhoujf Date:20180319 for:行编辑扩展参数字典值为空的问题
...
//	                                    + " onMouseOver=\"toolTip('<img src='+value+'>')\" onMouseOut=\"toolTip()\" "
	              ...
	              //update--end--author:zhangjiaqiang date:20170527 for:修订下载样式信息
					...
					//update--begin--author:zhangjiaqiang Date:20170815 for:TASK #2273 【demo】datagrid 多表头demo
					sb.append(",formatter:function(value,rec,index){");
					//update-begin-author:taoYan date:20180524 for:datagrid 操作列菜单改造--
					...
					sb.append("}");
					//update-end-author:taoYan date:20180524 for:datagrid 操作列菜单改造--
...
//----author:jg_longjb----start-----date:20150427--------for:新增封装查询器组件----
	...
	//update-end author:xuelin date:20171116 for:TASK #2404 【平台UI改造】UI样式改造点  5.参考fineui，做下字段过滤功能效果-----
	//update-begin-author:taoYan date:20180524 for:是否启用按钮过滤模式+操作列风格转变---
	...
	//update-end-author:taoYan date:20180524 for:是否启用按钮过滤模式+操作列风格转变---
	// update-begin-Author:LiShaoQing Date:20171227 for:添加新的高级查询构造器功能-----
	/**
	 * 高级查询构造器
	 * @param sb
	...
	private void addAdvancedQuery(StringBuffer sb,String buttonSytle) {
		//update-begin-author:taoyan date:2180522 for:复杂高级查询走外部弹框---
		/*FreemarkerHelper free = new FreemarkerHelper();
		Map<String, Object> mainConfig = new HashMap<String, Object>();
		...
		//update-end-author:taoyan date:2180522 for:复杂高级查询走外部弹框---
	}
	// update-end-Author:LiShaoQing Date:20171227 for:添加新的高级查询构造器功能-----
