    ...
    <div class="content">
        ...
        <div class="bottom-btn">
        	<c:forEach items="${transitionList}" var="trans">
        		<li><button class="btn btn-default" type="button" id="${trans.nextnode}" onclick="procPass('${trans.Transition }','${taskId}',${nextCodeCount},'${trans.nextnode}')" >${trans.Transition }</button></li>
			</c:forEach>
			<%-- <c:if test="${fn:contains(buttons, 'print')}" >
            	<li><button class="btn btn-default" id="print" type="button">打印</button></li>
            </c:if> --%>
            <c:if test="${nodeVariables.print == 'true'}" >
            	<li><button class="btn btn-default" id="print" type="button">打印</button></li>
            </c:if>
        </div>
    </div>
