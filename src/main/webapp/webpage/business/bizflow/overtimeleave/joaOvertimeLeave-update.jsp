...
<body>
	<t:formvalid layout="table" dialog="true" formid="formobj" usePlugin="password" action="joaOvertimeLeaveController.do?doUpdate" tiptype="6">
				...
				<tbody>
					...
					<tr>
						...
						<td width="233" valign="middle" style="padding: 4px; border-width: 1px; border-style: solid; background-color: rgb(228, 255, 219);" align="center" height="" data-fill="0" id="ext-gen3309">
							<input id="leaveTime" name="leaveTime" type="text" class="x-form-field tableform-field x-form-text"  datatype="*" value="${joaOvertimeLeave.leaveTime}"/>(天/小时)
						</td>
					</tr>
					<tr>
						<td width="153.66666666666666" valign="middle" style="padding: 4px; border-width: 1px; border-style: solid; word-break: break-all;" align="center">
						...
						<td width="226" valign="middle" style="padding: 4px; border-width: 1px; border-style: solid; word-break: break-all; background-color: rgb(228, 255, 219);" align="center">
							<input name="beginTime" type="text" style="background: url('plug-in/ace/images/datetime.png') no-repeat scroll right center transparent; width:100%;" 
								class="tableform-field" onClick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss'})" datatype="*" value="<fmt:formatDate value='${joaOvertimeLeave.beginTime}' type='date' pattern='yyyy-MM-dd HH:mm:ss'/>"/> 
						</td>
						<td width="153.66666666666666" valign="middle" style="padding: 4px; border-width: 1px; border-style: solid; word-break: break-all;" align="center">
						...
						<td width="233" valign="middle" style="padding: 4px; border-width: 1px; border-style: solid; word-break: break-all; background-color: rgb(228, 255, 219);" align="center">
							<input name="endTime" type="text" style="background: url('plug-in/ace/images/datetime.png') no-repeat scroll right center transparent; width:100%;" 
								class="tableform-field" onClick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss'})" datatype="*" value="<fmt:formatDate value='${joaOvertimeLeave.endTime}' type='date' pattern='yyyy-MM-dd HH:mm:ss'/>"/> 
						</td>
					</tr>
					<tr>
						<td width="140.66666666666666" valign="middle" style="padding: 4px; border-width: 1px; border-style: solid;" align="center">
					...
					<tr>
						...
						<td valign="middle" style="padding: 4px; border-width: 1px; border-style: solid; background-color: rgb(228, 255, 219);" align="left" rowspan="1" colspan="3">
							<input id="contactWay" type="text" size="1" name="contactWay" class="x-form-field tableform-field x-form-text"  style="width: 100%;" value="${joaOvertimeLeave.contactWay}">
						</td>
					</tr>
					<tr>
					...
					<tr>
						...
						<td valign="middle" style="padding: 4px; border-width: 1px; border-style: solid; background-color: rgb(228, 255, 219);" align="left" rowspan="1" colspan="3">
							<input id="workAgent" type="text" size="1" name="workAgent" class="x-form-field tableform-field x-form-text"  style="width: 100%;" value="${joaOvertimeLeave.workAgent}">
						</td>
					</tr>
					<tr style="height:300px;">
						<td valign="middle" colspan="4" rowspan="1" style="padding: 0px; border-width: 1px; border-style: solid;height:100px;">
							<t:datagrid actionUrl="joaOvertimeController.do?overtimeDatagrid&leaveId=${joaOvertimeLeave.id}" name="overtimeList" title="加班工时" fit="true" fitColumns="true" idField="id" checkbox="true" onDblClick="datagridDbclick">
								...
								<t:dgCol title="小时" field="applyHour" align="center" colspan="1" extendParams="editor:'text'"></t:dgCol>
   								<t:dgDelOpt title="删除" url="joaOvertimeDetailController.do?doDel&id={id}" urlclass="ace_button"  urlfont="fa-trash-o"/>
					...
					<tr>
						...
						<td width="226" valign="middle" style="padding: 4px; border-width: 1px; border-style: solid;" align="center" height="49">
							<input id="leaderRemark" type="text" size="1" name="leaderRemark" class="x-form-field tableform-field x-form-text" style="width: 100%;" value="${joaOvertimeLeave.leaderRemark}">
						</td>
						...
						<td width="233" valign="middle" style="padding: 4px; border-width: 1px; border-style: solid;" align="center" height="49" >
							<input id="deptLeaderRemark" type="text" size="1" name="deptLeaderRemark" class="x-form-field tableform-field x-form-text" style="width: 100%;" value="${joaOvertimeLeave.deptLeaderRemark}">
						</td>
					</tr>
				</tbody>
			</table>
...
<script type="text/javascript">
	function openSelectOrvertime() {
		...
		$.dialog({content: 'url:joaOvertimeLeaveController.do?openSelectOrvertime&leaveId='+leaveId, zIndex: getzIndex(), title: '加班单', lock: true, width: '1000px', height: '500px', opacity: 0.4, button: [
		 	{name: '<t:mutiLang langKey="common.confirm"/>', callback: callbackOvertimeSelect, focus: true},
		    {name: '<t:mutiLang langKey="common.cancel"/>', callback: function (){}}
		]}).zindex();
	}
	function callbackOvertimeSelect() {
		var iframe = this.iframe.contentWindow;
		var overtimeIds = iframe.getSelected();
		var leaveId = $("#id").val();
		$.ajax({
			url : "joaOvertimeDetailController.do?saveOvertimeIds",
			type : "POST",
			data : {
				leaveId : leaveId,
	...
	function datagridDbclick(index,field,value){
		$("#overtimeList").datagrid('beginEdit', index);
	}
	...
	function saveData(title,addurl,gname){
		if(!endEdit(gname))
			return false;
		var rows=$('#overtimeList').datagrid("getChanges","inserted");
		var uprows=$('#overtimeList').datagrid("getChanges","updated");
		...
		for(var i=0;i<rows.length;i++){
			for(var d in rows[i]){
				result["joaOvertimes["+i+"]."+d]=rows[i][d];
			}
		}
		$.ajax({
			url : addurl,
