		...
		$('#formtree').tree({
			...
			onClick : function(node) {
				if ($('#formtree').tree('isLeaf', node.target)) {
					...
					//update-begin--Author:zhoujf  Date:20180718 for：点击根节点查询所有Online表单类型
					loadFormByType("");
					//update-end--Author:zhoujf  Date:20180718 for：点击根节点查询所有Online表单类型
				}
			}
		});
	});
...
<t:datagrid sortName="createDate" sortOrder="desc" name="tablePropertyList" title="smart.form.config" fitColumns="true" actionUrl="cgFormHeadController.do?datagrid" idField="id" fit="true" 
	...
	<!-- update-begin-Author:LiShaoQing date:20180802 for:分辨率小的时候弹框看不全,改为100% -->
	...
	<!-- update-end-Author:LiShaoQing date:20180802 for:分辨率小的时候弹框看不全,改为100% -->
...
<script type="text/javascript">
	...
	function delCgForm(id,name){
		//update-begin--Author:gj_liucj  Date:20180427 for：TASK #2676 【Online开发】Online表单 删除时 提示信息不对--------------------
		$.dialog.confirm('<t:mutiLang langKey="confirm.online.delete.record"/>', function(){
			checkIsExit(id,name);
		}, function(){
		}).zindex();
		//update-end--Author:gj_liucj  Date:20180427 for：TASK #2676 【Online开发】Online表单 删除时 提示信息不对--------------------
	}
	//检查这个表是否已经存在了
	...
	function copyOnline(id){
		$.dialog.confirm('<t:mutiLang langKey="confirm.copy.form"/>', function(){
			$.post("cgFormHeadController.do?copyOnline",
					...
					if (d.success) {
						tip(d.msg);
						//update-begin-Author:LiShaoQing -- date:20180707 -- for:TASK #2921 【bug】点击online表单复制，提示成功，列表没刷新---
						reloadTable();
						//update-end-Author:LiShaoQing -- date:20180707 -- for:TASK #2921 【bug】点击online表单复制，提示成功，列表没刷新---
					}else{
						tip(d.msg);
					}
				});
