...
<t:formvalid formid="formobj" layout="div" dialog="true" action="processController.do?saveop">
	...
	<fieldset class="step">
        ...
        <div class="form">
            <label class="Validform_label"> <t:mutiLang langKey="common.auth.status"/>: </label>
            <select name="status">
	            <option value="1" <c:if test="${operation.status>0}"> selected="selected"</c:if>>
	                <t:mutiLang langKey="common.auth.status.y"/>
	            </option>
                <option value="0" <c:if test="${operation.status eq 0}">selected="selected"</c:if>>
                <t:mutiLang langKey="common.auth.status.n"/>
	            </option>
            </select>
        </div>
        ...
        <input name="processnodeId" value="${nodeId}" type="hidden">
