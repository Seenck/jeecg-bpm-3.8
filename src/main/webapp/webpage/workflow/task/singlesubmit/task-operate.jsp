...
.blueButton[disabled] {
  ...
  *background-color: #003bb3;
   ...
   <fieldset style="border: 1px solid #E6E6E6;">
 		<legend>表单信息</legend>
 		<div id="panelloadingDiv" class="panel-loading" width="100%">加载中....</div>
   		<iframe id="taskformiframe" name="taskformiframe"  src="taskController.do?goTaskForm&taskId=${taskId }" scrolling="no" frameborder="0" height="1%" width="100%" onload="hiddenloading();iframeresize('taskformiframe');"></iframe>
   </fieldset>
...
<!--							<a href="javascript:void(0)" class="jeecgDetail" onclick="del('tFinanceController.do?delFile&id=${bpmFile.id}',this)">删除</a>-->
			...
			<tr> 
		 	...
		 	<tr id="next_username_checkbox_tr" style="display:none"> 
			  <td class="value">
				 ...
				 <t:choose width="700" height="400" hiddenName="id" hiddenid="id" url="activitiController.do?goEntrust" name="entrusterList" icon="icon-search" title="common.user.list" textname="last" isclear="true"></t:choose><t:mutiLang langKey="common.noselect.default"/>
				 <!-- update-end--Author:zhoujf  Date:20180104 for：TASK #1215 【改造】会签支持配置固定的会签人员，不用再下一步节点的时候，手工选择 -->
			  </td>
		 	</tr>
		 	<tr> 
		 	<tr id="ccusername_checkbox_tr" style="display:none"> 
			  <td class="value">
				 ...
				 <!-- update-begin--Author:zhoujf  Date:20180104 for：TASK #1215 【改造】会签支持配置固定的会签人员，不用再下一步节点的时候，手工选择 --> 
				 <t:choose width="700" height="400" hiddenName="ccUserNames" hiddenid="id" url="activitiController.do?goEntrust" name="entrusterList" icon="icon-search" title="common.user.list" inputTextname="ccUserRealNames" textname="last" isclear="true"></t:choose>
				 <!-- update-end--Author:zhoujf  Date:20180104 for：TASK #1215 【改造】会签支持配置固定的会签人员，不用再下一步节点的时候，手工选择 -->
			  </td>
		 	</tr>
		 	<tr> 
			...
			<tr> 
			  <td class="value"  align="center">
			  		<div id="singleModel" style="display:black">
						...
						<c:forEach items="${transitionList}" var="trans">
							<input type="button" buttongroup="buttongroup" class="blueButton" onclick="disabledButton();preProcPass('${trans.Transition }','${trans.nextnode}')" value="${trans.Transition }">
						</c:forEach>
						<!-- update-begin--Author:zhoujf  Date:20170801 for：TASK #2225 去掉退回按钮，增加驳回上一步操作 -->
						<%-- <c:if test="${histListSize > 0 }">
						<c:if test="${turnbackTaskId!='' }">
			  		...
			  		<div id="manyModelButton" style="display:none">
			  			<input type="button" buttongroup="buttongroup" class="blueButton" onclick="disabledButton();preManyModelSubmit();" value='<t:mutiLang langKey="common.submit"/>'>
    ...
    function hiddenloading(){
    	...
    	function showButton(){
    		$('input[buttongroup="buttongroup"]').removeAttr("disabled");
    		$('input[buttongroup="buttongroup"]').attr("class","blueButton");
    	}
