...
//update-begin--Author:xuelin  Date:20170331 for：[#1714]【功能】角色分配菜单权限的时候，权限树采用ztree重写，不再采用easyui的树--------------------	
	var setting = {
		...
		async: {
			enable: true,
			url:"interroleController.do?setAuthority&roleId=${roleId}",
			dataFilter: filter				
		},
		callback: {
			...
			onClick: function (event, treeId, treeNode){
						...
						if (null == treeNode.children) {
							...
							/* $('#operationListpanel').panel(
									"refresh",
									"interroleController.do?operationListForFunction&functionId="+ treeNode.id + "&roleId=" + roleId);
							*/	
 						$('#dataRuleListpanel').panel(
									"refresh",
									"interroleController.do?dataRuleListForInterface&interfaceId="+ treeNode.id + "&roleId=" + roleId);
						} else {
						}
			}
		}
	...
	$(function() {	
		...
		$.fn.zTree.init($("#interfaceId"), setting);
		
		$("#functionListPanel").panel({
			title : '<t:mutiLang langKey="接口权限列表"/>',
			tools : [ {
				iconCls : 'icon-save',
				handler : function() {
					mysubmit();
	...
	function mysubmit() {
		var roleId = $("#rid").val();
		var s = GetNode();
		doSubmit("interroleController.do?updateAuthority&rolefunctions=" + s + "&roleId=" + roleId);
	}
	function GetNode() {
		var zTree = $.fn.zTree.getZTreeObj("interfaceId");				
		var node = zTree.getCheckedNodes(true);
		//加入实际被选中的节点
		var cnodes = '';
		for ( var i = 0; i < node.length; i++) {
